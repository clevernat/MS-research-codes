...
def timeseries(vars, pn, bts, bte,  lwp_time, pwv_time, aeri, time, id, st):
    ...
    #fig, ax = plt.subplots(x,2, figsize=(3,11/6*x))
    ...
    for i, v in enumerate(ax):
        ...
        if ps[0] >= 0:
            ...
            circle = ax[ps[0]][0].pcolormesh(vars['kazr']['time']-bts, vars['kazr']['h'], np.transpose(vars['kazr']['ref']),cmap=cmap,vmin=vmin,vmax=vmax,zorder=5)
            ...
            ax[ps[0]][0].set_ylim(0,ylim)
            ax[ps[0]][0].set_xlim(0,bt)
            ax[ps[0]][0].set_yticks(yticks)
            #ax[ps[0]].set_yticklabels(yticklabels, fontsize=13)
            ax[ps[0]][0].set_yticklabels(yticklabels, fontsize=5) #new
            ax[ps[0]][0].set_xticks(ticks)
            #ax[ps[0]].set_ylabel("MSL [km]", fontsize=13)
            ax[ps[0]][0].set_ylabel("MSL [km]", fontsize=7.2) #new
            if ps[0] == max(ps):
                #ax[ps[0]].set_xticklabels(labels, fontsize=10.1)
                ax[ps[0]][0].set_xticklabels(labels, fontsize=2.3, fontweight='bold') #new
                ax[ps[0]][0].tick_params(labelbottom=True,labelleft=True, labelright=True,bottom=True, top=True, left=True, right=True)
            else:
                ax[ps[0]][0].tick_params(labelbottom=False,labelleft=True, labelright=True,bottom=True, top=True, left=True, right=True)
            ax[ps[0]][0].yaxis.set_label_position("right")
            ax[ps[0]][0].tick_params('both', length=6, width=1, which='major')
    ...
    if ps[0] >= 0:
        ...
        y = vars['is']['h']
        cntr = ax[ps[0]][0].contour(vars['is']['time'][::2]-bts,y[::2],np.transpose(vars['is']['t'])[::2,::2],np.round(np.arange(-100,320,10)), colors='k', linewidths=0.8, linestyles ="dashed", zorder=6)
