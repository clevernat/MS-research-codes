...
for i, path in enumerate(path_to_variables[:]):
    ...
    # Retry processing the file if an error occurs
    retry_count = 0
    while retry_count < max_retry_attempts:
        try:
            ...
            if retry_count < max_retry_attempts:
                print(f"Retrying file '{output_file}' (Attempt {retry_count}/{max_retry_attempts})...")
                time.sleep(10)  # Wait for a minute before retrying
                continue
            else:
                print(f"Maximum retry attempts reached for file '{output_file}'. Moving to the next file.")
                break  # Move to the next file if maximum retries reached
